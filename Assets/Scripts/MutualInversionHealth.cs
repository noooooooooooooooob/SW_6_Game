...
public class HealthBarController : MonoBehaviour
{
    ...
    [SerializeField] private float changeRate = 5f;    // 고정 회복량 및 데미지량

    private float currentHealth;

    // 체력 비율에 따른 보너스 기준 (10%와 5% 비율로 설정)
    private const float HIGH_THRESHOLD = 0.1f;  // 10%
    private const float LOW_THRESHOLD = 0.05f;  // 5%
    ...
    public void Heal(float amount)
    {
        ...
        {
            amount += 2f; // 10%~5% 구간에서 추가 회복량
        }
        else if (healthPercentage <= LOW_THRESHOLD)
        {
            amount += 5f; // 5% 이하에서 최대 추가 회복량
        }
    ...
    public void TakeDamage(float amount)
    {
        ...
        if (healthPercentage <= HIGH_THRESHOLD && healthPercentage > LOW_THRESHOLD)
        {
            amount -= 2f; // 10%~5% 구간에서 데미지 감소
        }
        else if (healthPercentage <= LOW_THRESHOLD)
        {
            amount -= 3f; // 5% 이하에서 최대 데미지 감소
        }
